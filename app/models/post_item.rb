class PostItem < ApplicationRecord
  ...
  def create_notification_repost_item!(current_user)
    temp = Notification.where(["visitor_id = ? and visited_id = ? and post_item_id = ? and action = ? ", current_user.id, user_id, id, 'repost_item'])
    if temp.blank?
      notification = current_user.active_notifications.new(post_item_id: id, visited_id: user_id, action: 'repost_item')
      if notification.visitor_id == notification.visited_id
        notification.checked = true
      end
      notification.save if notification.valid?
